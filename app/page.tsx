...
type Application = { id: string; company: string; role: string; city: string; stage: string; appliedAt: string; nextAction: string; resumeId: string };
...
const seed: LocalData = {
  ...
  applications: [
    { id: "a1", company: "字节跳动", role: "产品经理", city: "北京", stage: "一面", appliedAt: "2026-08-08", nextAction: "今天 19:00", resumeId: "r-product" },
    { id: "a2", company: "腾讯", role: "商业分析", city: "深圳", stage: "笔试", appliedAt: "2026-08-10", nextAction: "明天 20:00 截止", resumeId: "r-analysis" },
    { id: "a3", company: "小红书", role: "用户增长", city: "上海", stage: "已投递", appliedAt: "2026-08-12", nextAction: "已等待 3 天", resumeId: "r-growth" },
  ],
};
...
export default function Home() {
  ...
  const addApplication = (event: FormEvent<HTMLFormElement>) => {
    ...
    const app: Application = { id: crypto.randomUUID(), company: String(form.get("company")), role: String(form.get("role")), city: String(form.get("city")), stage: String(form.get("stage")), appliedAt: String(form.get("appliedAt")), nextAction: String(form.get("nextAction")), resumeId: String(form.get("resumeId")) };
  ...
  const exportCsv = () => {
    const rows = [["公司","岗位","城市","阶段","投递日期","下一步","简历版本"], ...data.applications.map(a => [a.company,a.role,a.city,a.stage,a.appliedAt,a.nextAction,resumeName(a.resumeId)])];
